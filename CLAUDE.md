...
このプロジェクトでの作業を始める前に、Notionの「ブログ」データベース（database_id: `32d14e1c154580319050d297ace3ae7b`）を`API-post-search`等で検索し、タイトルが `[作業ログ]` から始まるページ（カテゴリー: その他）を確認すること。直近の作業内容・現在の状態・引き継ぎ事項がまとまっている。特にAdSense審査状況やnoteアーカイブの最新取り込み日など、コード上には残らない進行中タスクの状況はここにしかない。

セッションが長くなり引き継ぐ場合は、同じ形式で新しい`[作業ログ]`ページを作成し、このファイルの本セクションを更新すること。
